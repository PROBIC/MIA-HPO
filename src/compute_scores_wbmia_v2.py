...
class Learner:
    ...
    def run(self):
        ...
        self.models_dir = os.path.join(self.args.stats_dir, "Seed={}".format(self.args.seed),self.run_dir, self.exp_dir,"lira_models")
        ...
        # WB-OP Max Similarity
        opt_hypers_per_model_min = find_optimal_hypers(stats,in_indices)        
        self.scores["WBMaxSim"] = run_white_box_mia_sim(stats,in_indices,opt_hypers_per_model_min)
        # WB-OP Min Similary
        opt_hypers_per_model_max = find_optimal_hypers(stats,in_indices, min_or_max="maximum")    
        self.scores["WBMinSim"] = run_white_box_mia_sim(stats,in_indices,opt_hypers_per_model_max)
...
def find_optimal_hypers(stats, in_indices,min_or_max="minimum",metric = "hellinger"):
    ...
    for i in range(N_MODELS):
        ...
        for j in range(stats_shadow.shape[1]): # for the remaining columns compute overlap with target distribution/model
            ...
            curr_shadow_indices = np.vstack([in_indices[:i,:],in_indices[i+1:,:]])
            for k in range(curr_shadow_column.shape[0]):
                if metric == "hellinger":
                    overlaps[k] = hellinger_normal(stats_target[curr_shadow_indices[k]],curr_shadow_column[k,:,:][curr_shadow_indices[k]])
                elif metric == "carlini":
                    overlaps[k] = carlini_version(stats_target[curr_shadow_indices[k]],curr_shadow_column[k,:,:][curr_shadow_indices[k]])
                elif metric == "KL":
                    overlaps[k] = kl_divergence(stats_target[curr_shadow_indices[k]],curr_shadow_column[k,:,:][curr_shadow_indices[k]])
                else:
                    overlaps[k] = mean_difference(stats_target[curr_shadow_indices[k]],curr_shadow_column[k,:,:][curr_shadow_indices[k]])
