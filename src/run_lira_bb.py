...
class Learner:
    ...
    def run(self):
        ...
        target_model_base = self.args.target_stats_dir.split("/")[-2]
        ...
        # (HPO-ACC)MIA
        self.scores["CMIA"] = run_hpo_acc_mia(target_stats,shadow_stats, in_indices,use_global_variance = False)
        # (HPO-KL)MIA
        ...
        self.scores["MIA-KL"] = run_hpo_kl_mia(target_stats,shadow_stats,in_indices,opt_hypers_per_model_min)
       
        with open(os.path.join(self.target_stats_dir, 'scores_{}_{}_{}_T-{}_S-{}.pkl'.format(
                self.args.learnable_params,
                self.args.examples_per_class,
                self.target_epsilon,
                target_model_base,
                shadow_model_base)), 'wb') as f:
            pickle.dump(self.scores, f)   

        with open(os.path.join(self.target_stats_dir, 'opt_shadow_hypers_{}_{}_{}_T-{}_S-{}.pkl'.format(
                self.args.learnable_params,
                self.args.examples_per_class,
                self.target_epsilon, 
                target_model_base,
...
def run_hpo_acc_mia(target_stat, shadow_stat, in_indices, use_global_variance=False):
    N = target_stat.shape[0]
    n = target_stat.shape[2]
    
    cmia_target_stat = [] # get stats for diagonal models which serve as the target models
    for i in range(N):
        cmia_target_stat.append(target_stat[i,i,:,:])

    cmia_shadow_stat = [] # get stats for diagonal models which serve as the shadow models
    for i in range(N):
    ...
    for idx in range(N):
        ...
        stat_target = cmia_target_stat[idx]  # statistics of target model, shape (n, k)
...
def find_optimal_hypers(target_stat, shadow_stat,in_indices,metric = "KL"):
    ...
    for i in range(N_MODELS):
        ...
        stats_target = target_stat[i,i,:,:].flatten()
...
def run_hpo_kl_mia(target_stat,shadow_stat,indices,opt_hypers_per_model,use_global_variance=False):
    in_indices = np.array(indices)
    all_y_true, all_y_score = [],[]
    N_MODELS = target_stat.shape[0]
    for i in range(N_MODELS): 
        print(f"Target model M[{i}][{i}]")
        target = target_stat[i,i,:,:]
